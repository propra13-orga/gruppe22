...
namespace Gruppe22.Client
{
    public class AbilityChoice : Grid
    {
        ...
        private Backend.Ability[] _abilities;
        ...
        {
            Backend.AbilityElement element = Backend.AbilityElement.Health;// (AbilityElement)Math.Pow(2, _random.Next(10));
            Backend.AbilityTarget target = Backend.AbilityTarget.None;
            switch (element)
            {
                case Backend.AbilityElement.ManaReg:
                case Backend.AbilityElement.Health:
                case Backend.AbilityElement.HealthReg:
                    target = Backend.AbilityTarget.Self;
                    break;

                case Backend.AbilityElement.Fire:
                case Backend.AbilityElement.Charm:
                case Backend.AbilityElement.Scare:
                case Backend.AbilityElement.Stun:
                case Backend.AbilityElement.Ice:
                    switch (_random.Next(4))
                    {
                        case 0:
                            target = Backend.AbilityTarget.Explode;
                            break;
                        case 1:
                            target = Backend.AbilityTarget.Missile;
                            break;
                        case 2:
                            target = Backend.AbilityTarget.Aura;
                            break;
                        case 3:
                            target = Backend.AbilityTarget.Map;
                            break;
                    }
                    break;
                case Backend.AbilityElement.Morph:
                    ...
                    break;
                case Backend.AbilityElement.Teleport:
                    target = Backend.AbilityTarget.Map;
            ...
            int duration = 1;
            if (
                (element == Backend.AbilityElement.HealthReg)
                || (element == Backend.AbilityElement.Morph)
                || (element == Backend.AbilityElement.Charm)
                || (element == Backend.AbilityElement.Scare)
                || (element == Backend.AbilityElement.Stun)
                || (element == Backend.AbilityElement.ManaReg)
                || (element == Backend.AbilityElement.HealthReg))
                duration = _random.Next(5) + 5;
            int cooldown = 10 + _random.Next(10);
            int cost = 5 + intensity * duration + _random.Next(10) + (((target == Backend.AbilityTarget.Map) || (target == Backend.AbilityTarget.Explode)) ? 5 : 0);
            int improveOver = -1;
            string name = "";
            string description = "";
                ...
                if ((_actor.abilities[i].target == target) && (_actor.abilities[i].element == element))
                {
                    ...
                    do
                    {
                        switch (_random.Next(4))
                        {
                            case 0:
                                if ((duration < 12) &&
                                    ((element == Backend.AbilityElement.Health)
               || (element == Backend.AbilityElement.HealthReg)
                || (element == Backend.AbilityElement.Morph)
                || (element == Backend.AbilityElement.Charm)
                || (element == Backend.AbilityElement.Scare)
    ...
    , textwidth + 5, textheight + 5), new Rectangle(39, 6, 1, 1), new Color(Color.Black, 0.9f));
        ...
        #region Constructor
        public AbilityChoice(Backend.IHandleEvent parent, SpriteBatch spriteBatch, ContentManager content, Rectangle displayRect, Backend.Actor actor = null)
            : base(parent, spriteBatch, content, displayRect)
        {
            _actor = actor;
            _random = new Random(Guid.NewGuid().GetHashCode());
            _abilities = new Backend.Ability[3];
            GenerateAbility(0);
            GenerateAbility(1);
            GenerateAbility(2);
