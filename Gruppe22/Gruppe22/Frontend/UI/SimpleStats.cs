    ...
    public class SimpleStats : UIElement
    {
        ...
        public override void Draw(GameTime gameTime)
        {
            if (_actor != null)
            {
                ...
                // Character Name
                _spriteBatch.DrawString(_font, _actor.name.ToString(), new Vector2(_displayRect.Left + 9, _displayRect.Top + 4), Color.White);
                _spriteBatch.DrawString(_font, _actor.name.ToString(), new Vector2(_displayRect.Left + 10, _displayRect.Top + 5), color);
                
                // Separator Line
                _spriteBatch.Draw(_background, new Rectangle(_displayRect.Left + 5, _displayRect.Top + _lineheight + 10, _displayRect.Width - 10, 1), new Rectangle(39, 6, 1, 1), color);

                // Health bar
                _progressbar.Draw(gameTime); 

                // Statistics
                _spriteBatch.DrawString(_font, "ATK: " + _actor.damage.ToString() + " - DEF:" + _actor.armour.ToString(), new Vector2(_displayRect.Left + 10, _displayRect.Top + _lineheight * 3 + 20), color);
...
_displayRect.Left + 10, _displayRect.Top + _lineheight + 5, _displayRect.Width - 20, _lineheight + 10), ProgressStyle.Precise, (actor != null) ? actor.maxHealth : 0, (actor != null) ? actor.health : 0);
